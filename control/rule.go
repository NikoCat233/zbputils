...
func init() {
	...
	zero.OnCommandGroup([]string{"用法", "usage"}, zero.OnlyToMe, zero.AdminPermission).SetBlock(true).SecondPriority().
	...
	zero.OnCommandGroup([]string{"服务列表", "service_list"}, zero.OnlyToMe, zero.AdminPermission).SetBlock(true).SecondPriority().
		Handle(func(ctx *zero.Ctx) {
			gid := ctx.Event.GroupID
			if gid == 0 {
				gid = -ctx.Event.UserID
